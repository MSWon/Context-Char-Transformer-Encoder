...
def get_padding(seq_len, max_seq_length):
    mask = tf.sequence_mask(lengths=seq_len, maxlen=max_seq_length, dtype=tf.float32)
    with tf.name_scope("padding"):
        return 1-mask


def get_padding_bias(seq_len, max_seq_length):
  """Calculate bias tensor from padding values in tensor.
  Bias tensor that is added to the pre-softmax multi-headed attention logits,
  which has shape [batch_size, num_heads, length, length]. The tensor is zero at
  non-padding locations, and -1e9 (negative infinity) at padding locations.
  Returns:
    Attention bias tensor of shape [batch_size, 1, length, length].
  """
  with tf.name_scope("attention_bias"):
    padding = get_padding(seq_len, max_seq_length) ## batch_size, length
    padding = tf.expand_dims(tf.expand_dims(padding, axis=1), axis=1) ## batch_size, 1, 1, length
    padding = tf.tile(padding, [1,1,tf.shape(padding)[-1],1]) ## batch_size, 1, length, length
    padding = tf.linalg.set_diag(padding, tf.ones_like(padding[:,:,:,0])) ## batch_size, 1, length, length
