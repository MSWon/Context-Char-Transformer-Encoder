...
import numpy as np
import os
from CCTE.utils.data_utils import train_dataset_fn
from CCTE.utils import data_utils
from CCTE.model.electra import Electra
...
class Trainer(object):
    ...
    def __init__(self, hyp_args):
        ...
        self.word_vocab_path = os.path.join(path,hyp_args["word_vocab_path"])
        self.char_vocab_path = os.path.join(path,hyp_args["char_vocab_path"])
        ...
        data_utils.MAX_CHAR_LEN = hyp_args["max_char_len"]
        data_utils.MAX_WORD_LEN = hyp_args["max_word_len"]

        train_init_dataset = train_dataset_fn(self.corpus_path,self.word_vocab_path,
                                              self.max_word_len, self.mask_idx, self.batch_size)
        ...
        model = Electra(hyp_args, self.word_vocab_path, self.char_vocab_path)
        ...
        # self.clear_char_emb_padding = model.clear_char_emb_padding
        ## for tensorboard
    ...
    def train(self):
        ...
        with tf.Session(config=config) as sess:
            ...
            if (ckpt and tf.train.checkpoint_exists(ckpt.model_checkpoint_path)):
                saver.restore(sess, "./saved_model/" + self.model_path)
                print("Model loaded!")
            ...
            for step in range(self.training_steps):
                ...
                #sess.run(self.clear_char_emb_padding)
                # train
                ...
                if step % 10000 == 0 and step > 0:
                    saver.save(sess, "./saved_model/" + self.model_path)
