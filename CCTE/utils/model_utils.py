...
def get_padding_with_seq_len(seq_len, max_seq_length):
    mask = tf.sequence_mask(lengths=seq_len, maxlen=max_seq_length, dtype=tf.float32)
    with tf.name_scope("padding"):
        return 1-mask


def get_padding_bias_with_seqlen(seq_len, max_seq_length):
  with tf.name_scope("attention_bias"):
    padding = get_padding_with_seq_len(seq_len, max_seq_length) ## batch_size, length
